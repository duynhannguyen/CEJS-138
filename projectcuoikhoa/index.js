...
const initList = () => {
  ...
  for (let i = 0; i < saleClothListData.length; i++) {
    ...
    saleprice = Intl.NumberFormat("en-US").format(
      saleClothElement.saleprice
    );
...
const gearForMaleListData = [
  ...
  },
  {
    ...
    saleClothBrandName: "ADIDAS",
    saleClothProductName: "Áo Tập Nữ Adidas High Support Luxe Tailored Impact Training",
    price: 1800000,
    promo: 0.2,
    alt: "Đồ Tập Nữ ADIDAS",
    id: "GF8",
...
const addProductToCartGF = (string) => {
  const selectedProduct = gearForMaleListData.find((product) => product.id === string);
  const ProductInCartIndex = cart.findIndex((cartItem) => cartItem.id === string);
...
const addProductToCart = (string) => {
  const selectedProduct = menPoloListData.find((product) => product.id === string);
  const ProductInCartIndex = cart.findIndex((cartItem) => cartItem.id === string);
  if (ProductInCartIndex !== -1) {
    cart[ProductInCartIndex].quantity += 1;
  } else {
...
const addProductToCartSale = (string) => {
  const selectedProduct = saleClothListData.find((product) => product.id === string);
  const ProductInCartIndex = cart.findIndex((cartItem) => cartItem.id === string);
  if (ProductInCartIndex !== -1) {
    cart[ProductInCartIndex].quantity += 1;
  } else {
...
const initcartsale = () =>{
  ...
  const cartBodyItem = cart.map((cartItem) =>{
    const{saleClothImage, saleClothBrandName, saleClothProductName, price, quantity,alt,id,saleprice } = cartItem ;
    return `
...
const initcart = () =>{
  const cartpostion = document.querySelector(".cart .cart-content");
  const cartBodyItem = cart.map((cartItem) =>{
    const{saleClothImage, saleClothBrandName, saleClothProductName, price, quantity,alt,id } = cartItem ;
    return `
    <div class="cart-content-container" id="deleted-child">
    <div class="cart-img">
                <img src=${saleClothImage} alt=${alt}>
...
const cartTotal = () => {
  ...
  const cartContainner = cartContent.getElementsByClassName("cart-content-container");
  ...
  for(let i = 0; i<cartContainner.length ;i++){
    ...
    const QuantityEle = cartContainnerEle.getElementsByClassName("cart-quantity-input")[0];
  ...
  document.getElementsByClassName("totalNumber")[0].innerHTML = Intl.NumberFormat("en-US").format(Carttotal) + "đ";
...
const removeItemInCart = () =>{
  ...
  for(let i = 0 ; i < btnList.length ; i++ ){
    ...
    btnListEle.addEventListener("click", function(){
      //  const parent = document.querySelector(".cart-drop-down")
      const removeChild1 = document.querySelector(".cart-content-container")
      // const parent = document.querySelector("#parent")
      removeChild1.parentElement.removeChild(removeChild1);
      // console.log(removeChild1.parentElement);
        cart.pop(i);
        cartTotal()
...
// --------------------------------------------------------PURCHEASE---------------------------------------------------------//
const purchase = () => {
  alert('Bạn đã thanh toán thành công, đừng quên đăng ký email để nhận các chương trình khuyễn mãi mới nhất')
